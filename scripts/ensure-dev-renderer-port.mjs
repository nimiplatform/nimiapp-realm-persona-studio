...
function isStudioRendererProcess(commandLine) {
  const normalized = normalizeForMatch(commandLine);
  const normalizedStudioRoot = normalizeForMatch(studioRoot);
  return normalized.includes(normalizedStudioRoot)
...
function ensureRendererPortAvailable() {
  ...
  for (const pid of pids) {
    ...
    if (!isStudioRendererProcess(commandLine)) {
      throw new Error(
        `Port ${rendererPort} is already in use by PID ${pid}. ` +
        'It is not a recognized Realm Persona Studio renderer process, so cleanup was skipped.',
      );
    }

    console.log(`[dev-renderer-port] Stopping stale Studio renderer on port ${rendererPort} (PID ${pid}).`);
    process.kill(pid);
  }
}

...
export function buildPersonaCreationGraphFromDraft(
  ...
  const sourceFields = [
    ...
    sourceField('visibility', 'Visibility', present(draft.visibility), 'mapped', 'sourceProvenance'),
    sourceField('description', 'Profile description', present(draft.description), 'mapped', 'identity'),
    sourceField('greeting', 'Greeting', present(draft.greeting), 'mapped', 'greeting'),
  ...
  const sections: PersonaCreationGraphSection[] = [
    ...
    buildSection({
      key: 'greeting',
      title: 'Greeting',
      summary: draft.greeting ? 'The reviewed greeting is included in the persona profile.' : 'A greeting can be added now or after creation.',
      fields: field('Greeting', present(draft.greeting)),
      missing: [],
      risks: [],
  ...
  const writePlanItems: PersonaCreationGraphWritePlanItem[] = [
    {
      ...
      status: draft.handle && draft.displayName && draft.concept && draft.selectedWorldId && draft.visibility && draft.personaArchetype ? 'ready' : 'blocked',
    ...
    },
    {
      ...
      label: 'Review native profile after create',
      status: 'deferred',
      reason: 'The reviewed description is included in create; later changes use a complete PersonaCharacter replace.',
      ruleIds: ['R-RPS-GRAPH-017', 'R-RPS-GRAPH-020'],
    },
    {
